...
/**
 * Resolve path for read; tries filesystem variants (macOS NFD, curly quotes, etc.).
 */
export function resolveReadPath(filePath: string, cwd: string): string {
  ...
  const nfdCurly = tryCurlyQuoteVariant(nfdVariant);
  if (nfdCurly !== resolved && fileExists(nfdCurly)) return nfdCurly;
  return resolved;
}
